...
def test_profile_handler_fallback_to_custom():
    with patch("backend.profile_handler._fetch_access_list", return_value=[]), \
         patch("backend.profile_handler._fetch_profile", return_value=None), \
         patch(
         ...
         patch("backend.profile_handler._fetch_prompts", return_value=[]):
...
def test_profile_handler_not_found():
    with patch("backend.profile_handler._fetch_access_list", return_value=[]), \
         patch("backend.profile_handler._fetch_profile", return_value=None), \
         patch("backend.profile_handler._fetch_custom_profile", return_value=None), \
         patch("backend.profile_handler._fetch_metadata", return_value=None), \
         patch("backend.profile_handler._fetch_prompts", return_value=[]):
